...
import { Transfer, Table, Tag } from '@mlz/admin';
    ...
    {({ direction, filteredItems, onItemSelectAll, onItemSelect, selectedKeys: listSelectedKeys, disabled: listDisabled }) => {
      ...
      const rowSelection = {
        ...
        onSelectAll(selected, selectedRows) {
          const treeSelectedKeys = selectedRows.filter((item) => !item?.disabled).map((e) => e?.key);
...
const mockData = new Array(20).fill('').map((_, i) => ({
  key: i.toString(),
  title: `content${i + 1}`,
