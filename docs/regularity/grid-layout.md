...
import { Typography, Layout, Row, Col } from '@mlz/admin';
...
export default () => (
  <>
    ...
    <PageContainer>
        ...
        <Layout>
          ...
          <Content>
            <Row gutter={gutter} style={{ width: `${(contentWidth / layoutWidth) * 100}%`, margin: '0 auto', background: 'white' }}>
              {new Array(colNum).fill('').map((_, index) => (
                <Col className="gutter-row" span={2}>
                  <div style={style}>col-{index + 1}</div>
                </Col>
              ))}
            </Row>
          </Content>
      ...
      <Typography style={{ marginBottom: 80 }}>
        <Title level={4}>侧边导航布（左右布局，折叠后）</Title>{' '}
        <Layout style={{ position: 'relative' }}>
          <div className="fake-sider" style={{ width: fakeSiderCollapsedWidth, height: '100%', position: 'absolute', left: 0, top: 0, background: '#001529' }}></div>
          <Layout className="site-layout" style={{ marginLeft: fakeSiderCollapsedWidth }}>
            <Header style={{ width: '100%' }}>
              <div style={{ padding: '0 16px' }}></div>
            </Header>
            <Content style={{ padding: 24, background: '#FF7875' }}>
